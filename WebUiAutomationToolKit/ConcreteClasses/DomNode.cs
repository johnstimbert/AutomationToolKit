...
namespace WebUiAutomationToolKit
{
    internal sealed class DomNode : IDomNode
    {
        ...
        private Dictionary<string, object> FindAttributes(IWebDriverManager webDriverManager)
        {
            ...
            {
                throw new WebUiAutomationException($"Could not locate any element using the XPath {NodeSelector}");
            }
            catch (Exception ex)
            {
        ...
        private IWebElement GetElementReference(IWebDriverManager webDriverManager)
        {
            ...
            {
                throw new WebUiAutomationException($"Could not locate any element using the locator {NodeSelector}");
            }
            catch (Exception ex)
            {
                throw new WebUiAutomationException(ex.Message);
