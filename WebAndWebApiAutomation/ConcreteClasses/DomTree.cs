    ...
    internal sealed class DomTree : IDomTree
    {
        ...
        public void Build(SelectorData rootNode, IWebDriverManager webDriverManager)
        {
            try
            {
                ...
                if (root == null)
                    throw new WebAutomationException($"Could not locate an element using the locator {_rootNodeSelector}");
        ...
        private IWebElement GetElementReference(IWebDriverManager webDriverManager)
        {
            ...
            {
                throw new WebAutomationException($"Could not locate any element using the locator {_rootNodeSelector}");
            }
            catch (Exception ex)
            {
                throw new WebAutomationException(ex.Message);
